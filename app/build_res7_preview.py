    ...
    _run_with_query_progress(
        ...
        lambda: connection.execute(
            ...
            COPY (
                ...
                SELECT cells.h3_index, {columns}
                FROM cells
                JOIN species USING (gbif_accepted_id)
                GROUP BY cells.h3_index
                ORDER BY cells.h3_index
            ) TO {sql_path(building)} (FORMAT PARQUET, COMPRESSION ZSTD)
            """,
            [str(source)],
        ),
        phase="Aggregating",
...
def build_parts(args: argparse.Namespace) -> None:
    ...
    with BuildDisplay(
        ...
        work_weights={
            base_cell: sources[base_cell].stat().st_size
            for base_cell in requested
        },
...
def parser() -> argparse.ArgumentParser:
    ...
    aggregate.add_argument(
        "--threads", type=int, default=max(1, (os.cpu_count() or 2) // 2)
    )
