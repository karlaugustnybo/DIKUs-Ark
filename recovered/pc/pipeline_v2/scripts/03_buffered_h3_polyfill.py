#!/usr/bin/env python3
"""
03_buffered_h3_polyfill.py  —  Optimized Pipeline
...
# ---------------------------------------------------------------------------
H3_RES = 7
DENMARK_BBOX = (8.0, 54.5, 15.5, 57.8)
SLOW_TOL_DEG = 0.01
WRITE_BATCH = 200_000
...
# ---------------------------------------------------------------------------
def process_species(wkb: bytes, gbif_id: int, is_fast: bool) -> set[str] | None:
    try:
...
def _run_tasks_bounded(
    ...
    with ThreadPoolExecutor(max_workers=workers) as tpe:
        for _ in range(max_pending):
            ...
            fut = tpe.submit(process_species, wkb, gid, is_fast)
...
# Denmark filter
# ---------------------------------------------------------------------------
def _filter_denmark(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    dminx, dminy, dmaxx, dmaxy = DENMARK_BBOX
    wkb_values = df["geom"].to_numpy()
    try:
...
def _process_tasks(
    ...
    for gid, _, cells in _run_tasks_bounded(tasks, workers):
        if cells is None:
            errors += 1
            continue
        if cells:
...
def process_file(
    ...
    try:
        ...
        for batch in parquet_file.iter_batches(
            ...
            if denmark_only:
                before = len(df)
                df = _filter_denmark(df)
                denmark_kept += len(df)

                if before and len(df):
                    logger.info(
                        "Denmark batch: %d / %d kept",
                        len(df),
                        before,
                    )
    ...
    if denmark_only and denmark_kept == 0:
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="H3 Polyfill")
    parser.add_argument("--denmark-only", action="store_true")
    ...
    if args.files:
        ...
        if args.denmark_only:
            denmark_dir = H3_PARTS_DIR / "denmark"
            denmark_dir.mkdir(parents=True, exist_ok=True)
            done_names = {
                p.name.removesuffix("_h3.parquet")
                for p in denmark_dir.glob("*_h3.parquet")
            }
        else:
            done_names = {
                p.name.removesuffix("_h3.parquet")
                for p in H3_PARTS_DIR.glob("*_h3.parquet")
            }
    ...
    if args.denmark_only:
        logger.info("MODE: Denmark only")

    if args.denmark_only:
        denmark_dir = H3_PARTS_DIR / "denmark"
        outputs = [(f, denmark_dir / (f.stem + "_h3.parquet")) for f in input_files]
    else:
        outputs = [(f, H3_PARTS_DIR / (f.stem + "_h3.parquet")) for f in input_files]

    results = []

    for inp, out in outputs:
        r = process_file(inp, out, fast_lookup, args.denmark_only)
        results.append(r)
        # Incremental summary
        summary = {
        ...
        with open(H3_PARTS_DIR / "_processing_summary.json", "w") as fh:
